...
function ArtiestBandPut() {
    ...
    useEffect(() => {
      const fetchArtiestBand = async () => {
        try {
          const response = await fetch("https://localhost:7260/api/ArtiestBand");
          if (!response.ok) {
    ...
    useEffect(() => {
      const fetchBands = async () => {
        try {
          const response = await fetch("https://localhost:7260/api/band");
          if (!response.ok) {
      ...
      const fetchArtiesten = async () => {
        try {
          const response = await fetch("https://localhost:7260/api/artiest");
          if (!response.ok) {
    ...
    const handleSubmit = async (event) => {
      ...
      try {
        const response = await fetch(`https://localhost:7260/api/band/${bandId}/artist/${artiestId}`, {
          method: "PUT",
        });
        if (!response.ok) {
